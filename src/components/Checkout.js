  ...
  const Checkout = () => {
    ...
    const addAddress = async (token, newAddress) => {
      try {
        ...
        const response = await axios.post(`${config.endpoint}/user/addresses`, address, {
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    const performCheckout = async (token, items, addresses) => {
      ...
      if(validateRequest(items,addresses)){
          const prom=await axios.post(`${config.endpoint}/cart/checkout`,{"addressId":addresses.selected},{headers: {
            Authorization: `Bearer ${token}`,
            }
          });
          console.log(prom.data)
          if (prom.data.success){
    ...
    return (
      <>
        <Header data={localStorage.getItem("username")}/>
        <Grid container>
              ...
              <Box>
                {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */
                  ...
                  addresses.all.map((address)=>{
                    return (
                      <Box justifyContent="space-between" alignItems="center" key={address._id} display='flex' onClick={()=>{setAddresses({...addresses, selected:address._id})}} className={addresses.selected===address._id ? "selected":"not-selected"}>
                        <Typography>{address.address}</Typography>
                        <Button variant="text" onClick={()=>deleteAddress(token,address._id)}><Delete/> DELETE</Button> 
                      </Box>
                    )
                  })
                }
          ...
          <Grid item xs={12} md={3} bgcolor="#E9F5E1">
            ...
            <Typography variant="h5" sx={{fontWeight:"bold",mb:2}} >Order Details</Typography>
            ...
            <Grid container justifyContent="space-between" sx={{mb:1}}>
              ...
              <Grid container justifyContent="space-between" sx={{fontWeight:"bold",mb:1}}>
                <Typography sx={{fontWeight:"bold"}}> Total</Typography>
                <Typography sx={{fontWeight:"bold"}}> ${getTotalCartValue(items)}</Typography>
              </Grid>
            </Box>
          </Grid>
        </Grid>
