...
export const generateCartItemsFrom = (cartData, productsData) => {
  ...
  cartData.forEach((data)=>{
      ...
      if(data.productId===prodData._id){
        let copyProd={...prodData}
        copyProd['qty']=data.qty;
        newProdArr.push(copyProd);
      }
    })
  })
  return newProdArr;
...
const Cart = ({
  ...
  function handleAdd(item){
    handleQuantity(localStorage.getItem('token'),items,products,item._id,item.qty+1,{preventDuplicate:true});
  }
